...
public class FootSteps : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (controller.isGrounded && !prevGrounded && _audio.time > 0.25f)
        {
            _audio.volume = 1.5f;
            _audio.pitch = 1.2f;
            _audio.Play();
        ...
        if (controller.isGrounded && controller.velocity.y > 0.1f && _audio.time > 0.25f)
        {
            _audio.Stop();
            _audio.Play();
        }

        if (controller.velocity.x > 10.0f && _audio.time > 0.25f)
        {
            _audio.Stop();
            _audio.Play();
        }
