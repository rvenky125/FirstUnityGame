...
public class BundleDownloader : MonoBehaviour
{
    ...
    {
        var path = Application.persistentDataPath + "/" + name + ".unity3d";
        if (File.Exists(path))
        {
            Debug.Log("Loading asset");
            AssetBundle.LoadFromFile(path).LoadAsset(name);
            yield break;
        }
        else
        {
        ...
        using (UnityWebRequest web = UnityWebRequest.Get() .GetAssetBundle(url))
        {
